...
## D21 — 2026-09-02 — v19.0；以及 gate 有一个它自己查不出来的盲区

### 这一轮做完的

正文 3,968 词（限 4,000）。`[STALE]` 16 → **3**。Table 1/2/3 全部换成重跑值，
每一块都验过加和等于 3,997 / 15,523；Table 3 的星号由区间反推 p 得到，并自检
"星号必须与区间是否含 1 一致"，26 行全过。Limitations 并入 Discussion 收尾，不带小标题。

Supplement 删掉 **eTable 9、eTable 14、eFigure 3**：稿子里**没有任何一句引用它们**，
也不承担任何论断。其余重编号（10→9、11→10、11b→10b、12a/b/c→11a/b/c、13→12、15→13），
并做闭环校验——正文引用的 17 个编号全部解析到存在的 caption。

eTable 14 是被我自己的改写弄成孤儿的：它原来挂在 ¶77 的 AIDS 那句上，我重写 cross-cohort
段时把那句删了，表就没人引了。**删表之前先查引用，不要凭印象。**

### gate 的盲区

这一轮**两次**拿到"路径对、内容错"的文件：

1. `~/covid/repo/results/aou_v7/base_model_coefficients.csv`，mtime 是 Sep 2 01:41（很新），
   内容是 v18.6。重跑产物在桶里的 `data/covid_sdoh/aou_v7/`。
2. `table1_demographics.csv`，桶里那份是 v18.6，因为 `04_tables.py` 在 AoU 侧从来没跑过。
   跑完之后新表写在**本地** `results/aou_v7/`，与系数文件的新旧方向正好相反。

两次都是靠**内容指纹**发现的（女性 0.768 / Black 2.387 / pulmonary 0.904；表内 N=3,997）。
**任何基于路径或时间戳的核对都会放行。** 现在的 `gate.sh` 就是按路径取产物的，
所以它会给一份错的稿子开绿灯，而且开得理直气壮。

**修法（还没做）：** 在 gate 最前面加一段指纹断言，每个产物三个已知值，
对不上就直接失败。这比逐条断言便宜得多，而且挡的正是逐条断言挡不住的那类错误。

### 保留 eTable 11b（原 12b）的理由

Delta 窗口的衰减是 **−34.9%**——加进六个社会域之后，黑人种族估计从 1.78 升到 2.18。
有人会建议把这张表拿掉，因为它"不重要且和 thesis 无关"。

**不拿掉。** 它和 thesis 直接相关（RQ2 × RQ3），而且**把一个跑过、结果不好看的分析删掉，
正是这个负号本来会暴露的那种做法**。正确的处理是报出来，并说明为什么合并交互模型是主分析：
Delta 窗口 strata 最少，分波估计不稳。这句已经在正文和 supplement 首页里。

### 还欠

三处 `[STALE]`：Table 1 的三行波次计数、eTable 13（原 15）的 S3 失业下限、
有效样本量与 Omicron 收敛警告（`07e_matchit_summary.txt` 不在桶里）。
Figure 3–5 要按重跑系数重画；Figure 1–2 的 draw.io 两个面板由 PI 手改。
STROBE / RECORD 审查（stage 8）没做。
