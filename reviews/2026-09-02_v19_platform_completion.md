...
## 十、Figure 1 的 draw.io —— 改完了，顺带查出图注和 eTable 8 caption 两处错

`working/fig1_consort.drawio`（原文件备份为 `.bak`）。`efig2_phenotype_algorithm.drawio`
是纯决策树，**没有任何计数，不用改**。

MarketScan 那条链我没有信文档里的数，直接在 Quartz 上从 `results/ms/` 数了一遍。
数出来跟文档对不上：

| | 文档写的 | 数出来的 |
|---|---|---|
| Hospitalized | 139,472 | **139,489** |
| Outpatient | 4,283,335 | **4,283,711** |

正文 ¶168 的图注写 "139,472 hospitalized and 4,283,335 not"，**两个加起来是 4,422,807，
比它自己上一句的 4,423,200 少 393**——读者拿计算器一按就能发现。已改。

eTable 8 的 caption 跟着错：排除数写 11,571 例 / 401,608 对照，那是按旧的原始队列算的。
正确是 **11,588 / 401,984**（139,489 − 127,901，4,283,711 − 3,881,727），加起来 413,572
= 4,423,200 − 4,009,628。已改，并把原始队列的两个数写进 caption 让它自洽。

### 图里改了什么

**panel (a)**：匹配框 `Enrollment date` → **`Survey date`**；4,064 Cases → **3,997**；
15,856 Controls → **15,523 Control observations**（是观测数不是人数，配对是有放回的）；
**新增排除框**，镜像 panel (b) 已有的那个：67 例 + 284 对照因 index 前匹配变量不全被排除，
437 条对照观测因临近数据截止随访不全被排除。

**panel (b)**：139,472 → **139,489**；4,283,728 → **4,283,711**；139,468 Cases → **127,696**；
554,214 Controls → **509,983 Control observations**；排除框原来只写"4 Excluded"，
换成 11,588 例 + 401,984 对照（匹配变量不全）与 1,598 条对照观测 + 204 例（临近截止）。

两个 panel 的每一步加减现在都闭合，16 个等式全过。框的位置和大小我按 panel (b) 原来的
样式放的，**版式你调**。
