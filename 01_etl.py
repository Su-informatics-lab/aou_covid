#!/usr/bin/env python3
"""
COVID-19 Severity and Social Determinants of Health — Data Pipeline
All of Us Research Program, Controlled Tier

Extracts COVID-19 cohort, demographics, Charlson comorbidities, SDoH survey
responses, vaccination status, and performs propensity score matching.
...
CDR_VERSION = CDR.split(".")[-1]
TIMESTAMP = datetime.now().strftime("%Y%m%d")
PREFIX = f"covid_sdoh_{CDR_VERSION}_{TIMESTAMP}"
OUTDIR = "covid_sdoh"
RESULTS = "results"
os.makedirs(RESULTS, exist_ok=True)

print("=" * 70)
print("COVID-19 SEVERITY × SDoH DATA PIPELINE")
print("=" * 70)
print(f"  CDR:       {CDR}")
print(f"  BUCKET:    {BUCKET}")
print(f"  OUTPUT:    {RESULTS}/{PREFIX}_*.csv")
print("=" * 70)
...
def save(df, filename):
    ...
    subprocess.run(
        ["gsutil", "cp", filepath, f"{BUCKET}/data/{OUTDIR}/"], capture_output=True
    )
    print(f"  Saved: {RESULTS}/{filename} ({len(df):,} rows)")
...
save(covid_cohort, f"{PREFIX}_01_covid_cohort.csv")
...
save(demo_out, f"{PREFIX}_02_demographics.csv")
...
save(charlson, f"{PREFIX}_03_charlson.csv")
...
save(sdoh, f"{PREFIX}_04_sdoh.csv")
...
save(vacc_status[["person_id", "vaccination"]], f"{PREFIX}_05_vaccination.csv")
...
save(matched, f"{PREFIX}_06_matched_cohort.csv")
...
save(reg, f"{PREFIX}_07_regression_base.csv")

print("\n" + "=" * 70)
n_files = len([f for f in os.listdir(RESULTS) if f.endswith(".csv")])
print(f"PIPELINE COMPLETE — {n_files} files in {RESULTS}/ and {BUCKET}/data/{OUTDIR}/")
print("=" * 70)
