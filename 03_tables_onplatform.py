...
Usage: python 03_tables_onplatform.py aou_v7   # reads results/aou_v7/
       python 03_tables_onplatform.py ms        # reads results/ms/

Output: results/{cohort}/table1_demographics.csv  (aggregate, safe to export)
        results/{cohort}/table2_sdoh.csv           (aggregate, AoU only)
        Prints tables to stdout — paste into manuscript or chat.
"""
...
if len(sys.argv) < 2 or sys.argv[1] not in ("aou_v7", "aou_v8", "ms"):
    print("Usage: python 03_tables_onplatform.py [aou_v7|aou_v8|ms]")
    sys.exit(1)
...
print(f"Loaded {RESULTS}/07_regression_base.csv: {len(reg):,} rows")


# ── AoU cell suppression ─────────────────────────────────────────────
def safe_n(n, total):
    """Return count and percentage, suppressed if <20."""
    if n < 20:
        return "<20", ""
...
if "age_at_covid" in reg.columns:
    cm = cases.age_at_covid.mean()
    cs = cases.age_at_covid.std()
    km = ctrls.age_at_covid.mean()
    ks = ctrls.age_at_covid.std()
    rows.append(
        {
...
# ══════════════════════════════════════════════════════════════════════
# TABLE 2: SDoH Distributions (AoU only)
# ══════════════════════════════════════════════════════════════════════
sdoh_path = f"{RESULTS}/04_sdoh.csv"
matched_path = f"{RESULTS}/06_matched_cohort.csv"
...
if os.path.exists(sdoh_path) and COHORT.startswith("aou"):
    ...
    def sdoh_section(df_c, df_k, col, values, section_label):
        ...
        rows2.append(table_row(f"  Missing", int(cn), snc, int(kn), snk))
    ...
    for col, label in [
        ("ins_employer", "Employer"),
        ("ins_medicare", "Medicare"),
        ("ins_medicaid", "Medicaid"),
    ]:
        if col in s_cases.columns:
            cn = (s_cases[col] == 1).sum()
            kn = (s_ctrls[col] == 1).sum()
            rows2.append(table_row(f"  {label}", int(cn), snc, int(kn), snk))
